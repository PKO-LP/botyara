...
TOKEN = "8695975071:AAFN92TAAmfCWRnqiHxyMcj17JEHMe1IBv8"
CACHE_TTL = 600
...
async def corpus_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if corpus == "shenkurskoe":
        ...
        if not groups:
            await query.edit_message_text("❌ Не найдены группы.")
            return
...
async def group_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row = []
    for day in WEEKDAYS:
        row.append(InlineKeyboardButton(day[:3], callback_data=f"day_{day}"))
        if len(row) == 3:
...
async def day_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not group or not config_key:
        await query.edit_message_text("❌ Сначала выберите группу через меню.")
        return
    url = CORPUS_CONFIG[config_key]["url"]
    sheet = load_sheet(url)
    if not sheet:
    ...
    nav_buttons = [
        [
            InlineKeyboardButton("🔙 К дням", callback_data="back_to_days"),
...
async def back_to_groups(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    config_key = context.user_data.get('current_config')
    if not config_key:
        await query.edit_message_text("❌ Ошибка: конфигурация не выбрана. Начните с /start")
        return
    url = CORPUS_CONFIG[config_key]["url"]
    sheet = load_sheet(url)
    if not sheet:
        await query.edit_message_text("❌ Не удалось загрузить группы.")
        return
    groups = get_groups(sheet, CORPUS_CONFIG[config_key]["params"])
    if not groups:
        await query.edit_message_text("❌ Не найдены группы.")
